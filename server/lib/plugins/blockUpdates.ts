...
class ChunkUpdates {
	chunks: Map<string, { chunkX: number, chunkZ: number, updates: any }>
...
module.exports.server = (serv: MCServer, { version }) => {
	...
	const worldUpdateQueue: Map<World, { updateQueue: any[], updateSet: Set<string> }> = new Map()

	// We eliminate redundant block updates at this level
	// by checking if the update is already in the queue.
	const addUpdate = (updateQueue: any[], updateSet: Set<string>, update: { pos: Vec3, fromTick: any, tick: any, forceNotify: boolean, data: any }) => {
	...
	serv.updateBlock = (world: World, pos: Vec3, fromTick: any, tick: any, forceNotify: boolean = false, data: any = null) => {
	...
	serv.notifyNeighborsOfStateChange = (world: World, pos: Vec3, fromTick: any, tick: any, forceNotify: boolean = false, data: any = null) => {
		// TODO: it would be better to know the list of loaded worlds at initialisation
		if (!worldUpdateQueue.has(world)) {
			worldUpdateQueue.set(world, { updateQueue: [], updateSet: new Set() })
		}
	...
	serv.notifyNeighborsOfStateChangeDirectional = (world: World, pos: Vec3, dir: Vec3, fromTick: any, tick: any, forceNotify: boolean = false, data: any = null) => {
		// TODO: it would be better to know the list of loaded worlds at initialisation
		if (!worldUpdateQueue.has(world)) {
			worldUpdateQueue.set(world, { updateQueue: [], updateSet: new Set() })
		}
	...
	const updateHandlers: Map<number, (world: World, block: Block, fromTick: any, tick: any, data: any) => any> = new Map()
	/**
	 ...
	 */
	serv.onBlockUpdate = (name: string, handler: (world: World, block: Block, fromTick: any, tick: any, data: any) => any) => {
	...
	serv.on('tick', async (tickTime: number, curTick: number) => {
