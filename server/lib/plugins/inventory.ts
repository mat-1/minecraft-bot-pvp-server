...
import { MCItemEntity, MCPlayer, MCServer } from '../..'
...
module.exports.server = (serv: MCServer, { version }) => {
	...
	module.exports.player = function (player: MCPlayer) {
		const { InventoryWindow } = require('./windows')
		...
		interface SetCreativeSlotPacket {
			slot: number,
			item: {
				present: boolean,
				itemId: number,
		...
		player._client.on('set_creative_slot', ({ slot, item }: SetCreativeSlotPacket) => {
			if (player.gameMode !== GAMEMODES.creative) return // you have to be in creative to get stuff from creative
			if (item.itemId === -1) {
		...
		player.collect = (collectEntity: MCItemEntity) => {
