...
import { Anvil as mcAnvilInitializer } from 'prismarine-provider-anvil'

module.exports.server = async function (serv: any, { version, worldFolder, generation = { name: 'diamond_square', options: { worldHeight: 80 } }}: any = {}) {
	const mcData = mcDataInitializer(version)
	const Anvil = mcAnvilInitializer(version)
	...
	// serv.hub = new World(generationModule(generation.options), regionFolder === undefined ? null : new Anvil(regionFolder))
...
module.exports.player = function (player, serv, settings) {
	...
	player.changeWorld = async (world, { gamemode, dimension, difficulty, spawnpoint }: { gamemode?: string, dimension?: string, difficulty?: string, spawnpoint?: Vec3 }) => {
		...
		player._client.write('respawn', {
			...
			worldName: serv.dimensionNames[dimension || 0],
		...
		player.position = spawnpoint || player.spawnPoint
		player.sendSpawnPosition()
